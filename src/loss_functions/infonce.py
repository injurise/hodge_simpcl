...
class InfoNCE(Loss):
    ...
    def compute(self, anchor, sample, pos_mask, neg_mask, *args, **kwargs):
        ...
        log_prob = sim - torch.log(exp_sim.sum(dim=1, keepdim=True)) #broadcasting
        loss = log_prob * pos_mask
        loss = loss.sum(dim=1) / pos_mask.sum(dim=1)
        return -loss.mean()


class Sample_InfoNCE(Loss):
    # For now the same as the Var_WeightedInfoNCE class
    def __init__(self, tau):
        super(Sample_InfoNCE, self).__init__()
        self.tau = tau
...
class DebiasedInfoNCE(Loss):
    def __init__(self, tau, tau_plus=0.05):
        super(DebiasedInfoNCE, self).__init__()
        self.tau = tau
        self.tau_plus = tau_plus

    def compute(self, anchor, sample, pos_mask, neg_mask, *args, **kwargs):
        N = neg_mask.int().sum(dim=1,keepdim=True)
        M=pos_mask.int().sum(dim=1,keepdim=True)
        ...
        g = ((neg_sum/N) - (self.tau_plus * pos_sum)/M) / tau_minus
        g = torch.clamp(g, min= np.e ** (-1. / self.tau))

        log_prob = sim - torch.log((torch.exp(sim)*pos_mask + N*g.expand(-1,sim.shape[1])))
        loss = log_prob * pos_mask
        loss = loss.sum(dim=1)
        return -loss.mean()

class Sample_DebiasedInfoNCE(Loss):
    def __init__(self, tau, tau_plus=0.05):
        super(Sample_DebiasedInfoNCE, self).__init__()
        self.tau = tau
        self.tau_plus = tau_plus
...
class HardnessInfoNCE(Loss):
    ...
    def compute(self, anchor, sample, pos_mask, neg_mask, *args, **kwargs):
        ...
        reweighted_marginal = (exp_sim*neg_mask) * torch.exp(self.beta * sim)
        Z_beta_marg = (torch.exp(self.beta * sim)*neg_mask).sum(dim=1)/N
        marg_estimate= (reweighted_marginal/Z_beta_marg).sum(dim=1,keepdim=True)/N

        reweighted_pos = (exp_sim*pos_mask) * torch.exp(self.beta * sim)
        Z_pos_marg = (torch.exp(self.beta * sim)*pos_mask).sum(dim=1)/M
        pos_estimate = (reweighted_pos/Z_pos_marg).sum(dim=1,keepdim=True)/M

        ng =N* (marg_estimate - self.tau_plus * pos_estimate) / tau_minus
        ng = torch.clamp(ng, min=np.e ** (-1. / self.tau))

        log_prob = sim - torch.log((exp_sim * pos_mask + ng))
        loss = log_prob * pos_mask
        loss = loss.sum(dim=1) / pos_mask.sum(dim=1)
...
class Var_WeightedInfoNCE(Loss):
    def __init__(self, tau):
        super(Var_WeightedInfoNCE, self).__init__()
        self.tau = tau
...
class DebiasedInfoNCE_ADJ(Loss):
    def __init__(self, tau,n_pos_samples=1, tau_plus=0.05):
        super(DebiasedInfoNCE_ADJ, self).__init__()
        self.tau = tau
        self.tau_plus = tau_plus
        self.n_pos_samples =n_pos_samples

    def compute(self, anchor, sample, pos_mask, neg_mask,pos_sample_means, *args, **kwargs):
        N = neg_mask.int().sum(dim=1,keepdim=True)
        tau_minus = (1 - self.tau_plus)
        sim = _similarity(anchor, sample) / self.tau
        exp_sim = torch.exp(sim)
        ...
        neg_sum = (exp_sim * neg_mask).sum(dim=1,keepdim=True)
        g = ((neg_sum/N) - (self.tau_plus * pos_sample_means)) / tau_minus
        g = torch.clamp(g, min= np.e ** (-1. / self.tau))
