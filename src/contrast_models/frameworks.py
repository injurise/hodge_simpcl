#Basically got his from https://github.com/PyGCL/PyGCL/blob/main/GCL/models/contrast_model.py

import torch

from src.loss_functions.losses import Loss
from src.contrast_models import get_sampler
...
def add_extra_mask(pos_mask, neg_mask=None, extra_pos_mask=None, extra_neg_mask=None):
    ...
    if extra_neg_mask is not None:
        neg_mask = torch.mul(neg_mask,extra_neg_mask)
    else:
        neg_mask = 1. - pos_mask
...
class DualBranchContrast(torch.nn.Module):
    ...
    def forward(self, h1=None, h2=None, g1=None, g2=None, pos_sample_means= None,return_losses_sep=False, batch=None, h3=None, h4=None,
        ...
        if pos_sample_means != None:
            l1 = self.loss(anchor=anchor1, sample=sample1, pos_mask=pos_mask1, neg_mask=neg_mask1,pos_sample_means=pos_sample_means, **self.kwargs)
            l2 = self.loss(anchor=anchor2, sample=sample2, pos_mask=pos_mask2, neg_mask=neg_mask2,pos_sample_means=pos_sample_means, **self.kwargs)
        else:
