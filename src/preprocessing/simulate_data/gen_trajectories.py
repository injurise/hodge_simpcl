...
from src.preprocessing.simpl_complex import  SimplicialComplex
...
######################################################################################################
# This section was adopted from https://github.com/twitter-research/cwn/blob/main/data/datasets/flow_utils.py
# and from https://github.com/ggoh29/Simplicial-neural-network-benchmark/blob/main/OrientationFlow/OrientationFlowGraph.py
######################################################################################################
...
def generate_flow_cochain(class_id, G, B1, B2, T2):
    ...
    simp_comp = SimplicialComplex(X0 = torch.zeros((B1.shape[0], 1)), X1 = X1, X2 = torch.zeros((B2.shape[1], 1)),
                             B1 = B1, B2 = B2, labels = label)
    simp_comp.compute_hodge_laplacians()
    return simp_comp



def gen_orientation_graph(num_points=400, num_train=3, num_test=3,num_test_orient=3,
                          train_orientation='default', val_orientation='default', test_orientation="default", n_jobs=4):
    ...
    val_samples = Parallel(n_jobs=n_jobs, prefer="threads")(delayed(generate_flow_cochain) \
                                                                 (class_id=min(i // samples_per_class, 1), G=G, B1=B1,
                                                                  B2=B2,
                                                                  T2=get_orient_matrix(num_edges, val_orientation)) for
                                                             i in tqdm(range(num_test)))

    samples_per_class = num_test_orient // classes
    test_samples = Parallel(n_jobs=n_jobs, prefer="threads")(delayed(generate_flow_cochain) \
                                                                 (class_id=min(i // samples_per_class, 1), G=G, B1=B1,
                                                                  B2=B2,
                                                                  T2=get_orient_matrix(num_edges, test_orientation)) for
                                                             i in tqdm(range(num_test_orient)))
...
if __name__ == "__main__":
    ...
    train_data_path = os.path.join(ROOT_DIR,"data/trajectories/trajectories_cochains_train_test.obj")
    val_data_path = os.path.join(ROOT_DIR,"data/trajectories/trajectories_cochains_val_test.obj")
    test_data_path = os.path.join(ROOT_DIR,"data/trajectories/trajectories_cochains_test_test.obj")

    train_samples,val_samples,test_samples = gen_orientation_graph(num_points=400, num_train=200, num_test=200,num_test_orient=200,
                          train_orientation='default', val_orientation='default',test_orientation='default',
                          n_jobs=1)


    filehandler = open(train_data_path, "wb")
    pickle.dump(train_samples, filehandler)
    filehandler.close()

    filehandler = open(val_data_path, "wb")
    pickle.dump(val_samples, filehandler)
    filehandler.close()

    filehandler = open(test_data_path, "wb")
